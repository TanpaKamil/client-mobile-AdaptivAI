...
export default function MyModuleScreen() {
  ...
  const { instanceModules, fetchInstanceModules } = useModules();
  const { user } = useAuth();
  const navigation = useNavigation();

  useEffect(() => {
    fetchInstanceModules();
    console.log(instanceModules);
  }, [])
      ...
      <View style={{ flex: 1, marginTop: 60, width: "100%" }}>
        <View style={MyModuleStyles.mainContainer}>
          ...
          <View style={MyModuleStyles.btnGenerate}>
            <ButtonGenerate text={"Create New Module"} onPress={() => navigation.navigate("Generate")}/>
          </View>
          ...
          <View style={MyModuleStyles.columnContainer}>
            ...
            <View style={{ height: "88%" }}>
              <FlatList
                data={instanceModules}
                renderItem={({ item }) => <UserModuleCard module={item} key={item}/>}
              />
            </View>
          </View>
        </View>
